...
function card({ id, image, title, description, range, transmission, fuel_type, year, cost }: TCard) {
  ...
  return (
    <Link to={productPath}>
      <div className='flex-none w-[300px] min-h-[380px] p-4 bg-white rounded-lg hover:shadow-md mx-4 mb-2 border-[0.5px] border-slate-200'>
        <div className=''>
          <img className='w-full h-32 object-contain mb-4 rounded-lg' src={image} alt={title} />
        </div>
        <div className=''>
          <h3 className="text-[#050b20] text-lg font-medium font-['DM Sans'] leading-[2]">{title}</h3>
          <p className="text-[#050b20] text-sm font-normal font-['DM Sans'] leading-[14px]">{description}</p>
        </div>
        <div className='mt-4'>
          ...
          <div className='inline-flex justify-between w-full'>
            <div className="text-[#050b20] text-lg font-bold font-['DM Sans'] leading-[30px]">{cost} VND</div>
