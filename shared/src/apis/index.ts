...
import { LoginBody, RegisterBody, TProduct } from './types';
...
// region --------------------------Auth------------------------------------
export const register = (body: RegisterBody) => API.post(`${BASE(ver_API, ROUTES_AUTH)}/register`, body);

export const login = (body: LoginBody) => API.post(`${BASE(ver_API, ROUTES_AUTH)}/login`, body);

export const logout = (refreshToken: string) => API.post(`${BASE(ver_API, ROUTES_AUTH)}/logout`, { refreshToken });

export const sendEmailOtp = (email: string) => API.post(`${BASE(ver_API, ROUTES_AUTH)}/verify/otp`, { email });
...
export const updateAccountInfo = (data: any) => API.patch('/account', data);

export const changePassword = (data: any) => API.patch('/account/change-password', data);

export const getAddresses = () => API.get('/account/addresses');

export const addAddress = (data: any) => API.post('/account/addresses', data);

export const updateAddress = (id: string, data: any) => API.patch(`/account/addresses/${id}`, data);

export const deleteAddress = (id: string) => API.delete(`/account/addresses/${id}`);
...
export const getAllProduct = (params: string) => API.get('/products', { params });
...
export const getListBank = () => API.get(`${PAYOS_LISTS_BANK_URL}`);

export const getOrder = () => API.get(`${PAYOS_ORDER_URL}`);

export const cancelOrder = (orderId: unknown) => API.post(`${PAYOS_ORDER_URL}/order/${orderId}`);
